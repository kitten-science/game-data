...
const main = async () => {
	const literalsRaw = readFileSync(
		join(gameRoot, "./res/i18n/en.json"),
		"utf8",
	);
	...
	Object.assign(
		globalThis,
		{
			$I: (literal: string, ..._args: Array<unknown>) =>
				literals[literal] ?? "",
			dojo: {
				declare: (
					id: string,
					...
					switch (id) {
						case "classes.managers.Achievements":
							dumpAnyToFile(
								"achievements",
								metadataToHash(mustExist(decl.achievements)),
							);
						...
						case "classes.managers.BuildingsManager":
							dumpAnyToFile(
								"buildings",
								metadataToHash(mustExist(decl.buildingsData)),
							);
							break;
						...
						case "classes.managers.ReligionManager":
							dumpAnyToFile(
								"zigguratUpgrades",
								metadataToHash(mustExist(decl.zigguratUpgrades)),
							);
							dumpAnyToFile(
								"religionUpgrades",
								metadataToHash(mustExist(decl.religionUpgrades)),
							);
							dumpAnyToFile(
								"transcendenceUpgrades",
						...
						case "classes.managers.ScienceManager":
							dumpAnyToFile(
								"policies",
								metadataToHash(mustExist(decl.policies)),
							);
						...
						case "classes.managers.SpaceManager":
							dumpAnyToFile(
								"programs",
								metadataToHash(mustExist(decl.programs)),
							);
						...
						case "classes.managers.TimeManager":
							...
							dumpAnyToFile(
								"voidspaceUpgrades",
								metadataToHash(mustExist(decl.voidspaceUpgrades)),
							);
							break;
						...
						case "classes.managers.WorkshopManager":
							...
							dumpAnyToFile(
								"upgrades",
								metadataToHash(mustExist(decl.upgrades)),
							);
							dumpAnyToFile(
								"zebraUpgrades",
								metadataToHash(mustExist(decl.zebraUpgrades)),
							);
							break;
					}
				},
